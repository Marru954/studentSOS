...
Aggiornato: 2026-09-26 (septies: aggiunta la "Coda task" leggibile dagli agenti; prima: `safe-merge.sh --auto` ripristinato + tracking atenei; regole v2, un solo muro, PR #41)
...
- [ ] stableid-nul-escape | area:chore | auto:si | motivo-no:- | batch:- | note: **in corso (auto/stableid-nul-escape-2026-09-26)**; in `src/lib/sync/util.ts` sostituire il NUL letterale con l'escape `"\0"` (stessa stringa → id invariati; verificarlo con i test esistenti su `stableId`); non toccare `easyacademy.ts`/`engine.ts`
