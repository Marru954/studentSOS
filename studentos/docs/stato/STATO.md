...
Aggiornato: 2026-09-26 (octies: supervisore automatico, task `stableid-nul-escape`; prima — septies: aggiunta la "Coda task" leggibile dagli agenti; prima: `safe-merge.sh --auto` ripristinato + tracking atenei; regole v2, un solo muro, PR #41)

## Completati
### Sessione 2026-09-26 (octies) — supervisore automatico: `stableid-nul-escape` (branch auto/stableid-nul-escape-2026-09-26, PR da mergiare dal proprietario)
✅ `src/lib/sync/util.ts`: il separatore di `stableId` era un byte NUL scritto nel sorgente (grep e git trattavano il file come binario); ora è l'escape `"\0"`, **stessa stringa a runtime**. Verifica reale: 8 input calcolati prima e dopo la modifica con `tsx` → output identico (`["1gs0z0knzjw","vpi2f03p5a","wmao3leoz1","xb319mxp3m","49fp6jq86t","wvjl67o803","2bjocenw4cc","fwu8vjuwsl"]`, `cmp` → IDENTICI); `file` ora dice "JavaScript source, UTF-8 text", 0 byte NUL.
✅ Nuovo `tests/stableIdGolden.test.ts` (2 test, registrato nello script `test`): fissa quegli 8 id di riferimento e controlla che il separatore resti (`['a','b']` ≠ `['ab']`), così un cambio futuro che rinumera gli id in IndexedDB viene scoperto.
✅ Gate verde: build (Next 16.3.6), test 519/519, tsc, lint. Revisioni: code-review medium → nessun finding; security-reviewer → nessun problema (ha confermato con hex-dump che il vecchio byte era `0x00`: separatore invariato, nessuna superficie SSRF/auth/rate-limit toccata).
📝 Nota ambiente: nel worktree un collegamento (junction) a `node_modules` del checkout principale fa fallire Turbopack ("Symlink … points out of the filesystem root") e quel `node_modules` è fermo a Next 16.2.9: nei worktree serve `npm ci`.

### Sessione 2026-09-26 (sexies) — auto-merge `safe-merge.sh --auto` + tracking atenei (branch chore/auto-merge-gate)
🔀 **Riconciliazione con le regole v2**: il lavoro è nato prima della PR #41, che (mentre lavoravo) ha cancellato `scripts/safe-merge.sh` e il wrapper su `main` ("da adesso tutto automatico"). Fermato senza mergiare, ho riportato il conflitto all'utente con 3 strade; **ha scelto A: ripristinare lo script con `--auto`**. Ricostruito il branch su `origin/main`; lo script è byte-identico a quello testato (sha1 `8777f553b33d`); il wrapper `studentos/scripts/safe-merge.sh` NON è ripristinato (CLAUDE.md corretto); tornano nello script i muri #4/#5 che la #41 aveva reso linee guida (gli hook PreToolUse restano rimossi).
✅ **`./scripts/safe-merge.sh --auto`**: gate verde + muri #4/#5 verdi + nessun file sensibile → tag `rollback/<data-ora>` → merge → push, senza fermarsi. Gate o muri rossi: si ferma (exit 1), nessun tag, nessun merge. **Senza flag lo script è identico a prima**: diff = 324 righe aggiunte, 0 rimosse; stesso output e codice di uscita dell'originale su 3 scenari (branch pulito, tocca un preset, muro #4 rosso). Unica differenza voluta: un argomento sconosciuto (es. `--atuo`) è rifiutato (exit 2) invece che ignorato.
✅ **Eccezione obbligatoria** (exit 10, "AUTO-MERGE FERMATO", elenco dei file che hanno fatto scattare il blocco, anche a gate verde): rilevamento su 3 livelli sul diff `origin/main...HEAD` con `--no-renames` (un file spostato o cancellato compare col vecchio nome), sempre case-insensitive: (1) **percorso** — modulo `src/lib/sync/**` (engine, adapter, preset atenei, codici, note coverage), route `api/**/sync*`, discovery/sync insegnamenti, `syncClient.ts`, `state/synced.ts`, `state/settings.ts` (`enabledSources`), `emailToAteneo.ts`, tool `audit-/recapture-/probe-*`, `verified-endpoints.txt`; (2) **contenuto** — righe aggiunte/rimosse (fuori dai `.md`) che citano easyacademy/easystaff/agendaweb/combo.php/grid_call/test_call/anno2/livePrograms/degreeSources/enabledSources/cineca…; (3) **link** — qualunque symlink/submodule. **Fail-closed**: le due tabelle di regole devono restare allineate, ogni regola deve trovare almeno un file su `origin/main` (se qualcuno sposta `src/lib/sync/` la regola diventa obsoleta e blocca invece di proteggere il nulla), errore di git o diff vuoto = blocco.
✅ **Estensione decisa in sessione**: stesso blocco per le **guardie del merge** (`safe-merge.sh`, `.claude/settings.json`, `studentos/scripts/hooks/`, dove vive il muro rimasto `check-invented-data`). Senza, un branch potrebbe modificare la propria regola e auto-mergiarsi, rendendo l'eccezione rimovibile in silenzio.
✅ **Riepilogo a ogni uscita** (merge, blocco o errore, via `trap`): ~10 righe a schermo + `studentos/docs/stato/LAST_SESSION_SUMMARY.md` (gitignored: riscritto a ogni corsa, non sporca l'albero). Dice esito, branch, cosa entra, gate passo per passo, muri, file sensibili, effetti, tag di rollback e comando di ripristino, eventuali override `ALLOW_*` attivi.
✅ **Test con output reale**, in un clone isolato con `origin` locale (nessun commit di prova sul vero `main`): (a) branch pulito, gate vero → MERGIATO, exit 0, `main` +1 commit, tag su origin, albero pulito; (b) branch che tocca `unifi.ts`, gate vero verde → BLOCCATO, exit 10, `main` e tag invariati; (c) errore di tipo → gate rosso, exit 1, nessun tag/merge. Poi: branch rimasto indietro (nessun falso allarme per le modifiche già su `main`, merge commit invece di fast-forward) e merge manuale dopo il blocco (senza `--auto` passa). **Matrice di 41 casi** in DRY_RUN: 33 bloccano come atteso (percorso in MAIUSCOLO, file rinominato/cancellato/solo-chmod, nome con spazi e accenti, symlink, marker in un file fuori perimetro), 7 negativi passano (UI, docs con "EasyAcademy", dominio, test senza marker…), 1 diff vuoto blocca; più regola obsoleta, tabelle sfasate e storia non collegata → blocco fail-closed.
✅ **`docs/stato/ATENEI_TRACKING.md`**: 18 atenei live + Bari (manuale) con anno verificato, stato sorgenti, data dell'ultimo lotto, trigger del prossimo, note; eccezioni **Bari** (combo 2026 vuoto, 0 celle → manuale, wiring 2025/26 conservato) e **Unife** (14 percorsi abilitanti su `anno` 2025: con 2026 danno 0 celle). Conteggi letti dal codice (1.484 programmi, 3.345 orari, 2.489 esami = `LIVE_PROGRAMME_COUNT`); nessuna data futura inventata.
⚠️ **Limiti del rilevamento (onesti)**: è affidabile *per il perimetro definito*, non "al 100%" in assoluto. Una modifica che cambia il comportamento del sync in un file fuori dai perimetri **e** in righe senza nessuno dei marcatori non viene vista. Trovato e chiuso al secondo passaggio: `state/settings.ts` (`enabledSources`) era fuori. Il percorso *manuale* (senza `--auto`) non ha questi controlli: è la via "dopo conferma umana".
...
✅ Ridotti i muri assoluti da 6 a 1 (resta "mai inventare dati", hook `check-invented-data.mjs`, invariato e riprovato con payload simulati: blocca su `auto/*`, avvisa altrove). Cancellati gli hook `check-protected-files`, `check-main-protection`, `check-new-deps` con le loro entry in `.claude/settings.json` (resta anche l'hook SessionStart di main); cancellati `scripts/safe-merge.sh` e il wrapper `studentos/scripts/safe-merge.sh` (decisione esplicita dell'utente: "da adesso tutto automatico"); rimosso `tests/hooks-muri-1-2.test.ts` (copriva solo gli hook tolti) e la parte hook-B di `tests/hooks-muri.test.ts` (8 test di `check-invented-data` restano verdi), riga tolta dallo script `test` di `package.json`. `.github/workflows/claude.yml`: il passo "Verifica muri" ora controlla solo `check-invented-data` (senza, ogni esecuzione di `@claude` sarebbe fallita). Gate verde prima della PR: build, test, tsc, lint; CI della PR verde (2 check).
🐞 Errore mio, corretto nella stessa PR: riscrivere il workflow con `Set-Content -Encoding utf8` (PowerShell 5.1) aveva aggiunto un BOM e rotto gli accenti nei commenti; ripristinato da main e rifatto con `[IO.File]::WriteAllText(..., UTF8Encoding($false))` (diff finale: 1 riga).
⚠️ **Ancora da fare**: (1) `CLAUDE.md` non è aggiornato: manca la sezione "Regole del progetto — 3 livelli" (testo in `regole-progetto-v2-2026-09-26.md`) e le sezioni "Permanent rules" e "Git workflow" citano ancora `safe-merge.sh`, i muri #4/#5 come hook e il divieto di push su main; il classificatore dell'ambiente ha bloccato le modifiche a CLAUDE.md, servono permessi espliciti o modifica a mano. (2) Project instructions su claude.ai da riallineare a mano. (3) `docs/stato/CONTESTO_TECNICO.md` cita ancora `safe-merge`. (4) Il branch `claude/easyacademy-preset-codes-2026-b21e85` (68 commit dietro main) ha 11 file in conflitto con main, già prima della #41 (non causati da questa PR): da archiviare o riportare a mano.
📝 Nota operativa: dopo la v2 nulla blocca più per hook file core / main / dipendenze; vale la regola "motivarlo nel report".

### Sessione 2026-09-26 (quater) — portata su main l'ultima voce di diario rimasta fuori (branch claude/gifted-carson-nkr5yq, solo STATO.md)
✅ Richiesta utente "fai automerge": nessuna PR aperta. Controllati tutti i branch remoti con `git cherry` contro main: l'unico lavoro non ancora su main era 1 commit di `claude/ricattura-codici-2026-uyvllt` (solo STATO.md, `de63f00`). Portato con `cherry-pick -x` (il muro #2 vieta qualunque `git merge`, anche sui branch di lavoro) e conflitto risolto come previsto il 25-09: tenute entrambe le voci, la sua rinominata "nonies" (v. sotto). Nessun codice toccato.
🔎 Tutti gli altri branch remoti hanno 0 commit fuori da main, tranne `auto/hook-muri-4-5` (obsoleto: il suo contenuto è su main dal 2026-07-04; cancellazione autorizzata ma bloccata dal proxy, v. sessione dipendenze). Dopo questo merge anche `claude/ricattura-codici-2026-uyvllt` è un residuo da cancellare.
...
### Sessione 2026-09-25 (nonies) — tentativo ri-cattura lotto Tor Vergata/Firenze/Federico II, bloccato dalla rete (0 commit sui preset; branch claude/ricattura-codici-2026-uyvllt, portato su main il 2026-09-26)
...
## In sospeso
- **Auto-merge — regola d'uso per le sessioni future**: lanciare `./scripts/safe-merge.sh --auto` sui branch ordinari; se esce **10**, mostrare all'utente l'elenco dei file e rilanciare senza `--auto` solo dopo un sì esplicito in chat. Il branch che ha introdotto `--auto` toccava le guardie e ha avuto il sì esplicito dell'utente (scelta A) prima del merge.
- **Test automatico del rilevamento file sensibili**: oggi provato a mano (41 casi + 3 end-to-end con gate vero, sessione sexies), non salvato in `tests/`; richiede una riga in `package.json`.
- **Tracking atenei**: `docs/stato/ATENEI_TRACKING.md` va aggiornato a ogni lotto verificato (i numeri si rileggono da `UNIVERSITY_PRESETS`); nessuna data di lotto futuro è fissata, solo i trigger (sessione esami aperta, orario 2026/27 di Tor Vergata, combo 2026 di Bari, settembre 2027).
...
- **Pulizia branch remoti — a mano su GitHub** (il proxy di questo ambiente blocca le cancellazioni): residui con contenuto già su main `auto/hook-muri-4-5` (cancellazione autorizzata il 2026-09-26) e `claude/ricattura-codici-2026-uyvllt`; già mergiati per intero `chore/gh-action-claude`, `claude/project-overview-suspended-ca1248`, `claude/recon-cineca-gomp-universities-1poxh2`, `claude/safe-merge-script-a3w3ff`, `claude/safemerge-enforcement-walls-a9vmiz`, `claude/session-start-stato-hook-285a37`, `docs/riorganizza-md`. Nessuna urgenza: non danno fastidio al codice.
...
- ~~Igiene: NUL letterale in `src/lib/sync/util.ts`~~ **FATTO il 2026-09-26 (octies)**: escape `"\0"`, id invariati, test `stableIdGolden`.
...
- [x] stableid-nul-escape | area:chore | auto:si | motivo-no:- | batch:- | note: **fatto (auto/stableid-nul-escape-2026-09-26, PR in attesa del proprietario)**; in `src/lib/sync/util.ts` sostituire il NUL letterale con l'escape `"\0"` (stessa stringa → id invariati; verificarlo con i test esistenti su `stableId`); non toccare `easyacademy.ts`/`engine.ts`
...
- **26/09/2026 (supervisore automatico) — `stableid-nul-escape`: aggiunto un test nuovo di valori di riferimento invece di affidarsi ai soli test esistenti su `stableId`** (quelli controllano determinismo e differenze, non i valori): gli id sono chiavi IndexedDB, un cambio silenzioso rinumererebbe le cache. Nessun'altra scelta di merito.
- **26/09/2026 — auto-merge attivo (`scripts/safe-merge.sh --auto`) su fix/feature/adapter/chore; ECCEZIONE OBBLIGATORIA: un branch che tocca file di preset/config EasyAcademy o il modulo di sync verso le università si ferma SEMPRE per conferma manuale (exit 10), anche a gate verde e a prescindere dal tipo di branch.** Motivo: "dati non inventati / verificato con output mostrato" non è una cosa che build+test+tsc+lint possano controllare; un preset sbagliato che passa il gate è dato falso in produzione. Autorizzazione: esplicita e specifica dell'utente il 26/09/2026 a modificare `scripts/safe-merge.sh` — vale solo per questa modifica, non è estendibile ad altre; l'eccezione non si rimuove senza una nuova autorizzazione esplicita. **Riconciliata con "regole v2" (un solo muro)**: la PR #41 aveva cancellato lo script lo stesso giorno; l'utente ha scelto di ripristinarlo con `--auto` (opzione A; le alternative erano un controllo lato GitHub o lasciar perdere). Estensione decisa in sessione: stesso blocco per le guardie del merge (lo script, `.claude/settings.json`, `studentos/scripts/hooks/`), altrimenti un branch potrebbe togliere l'eccezione e auto-mergiarsi. Senza `--auto` lo script resta identico a prima (nessun controllo aggiunto: è la via "dopo conferma umana"). Perimetro e limiti del rilevamento: v. sessione 2026-09-26 (sexies).
